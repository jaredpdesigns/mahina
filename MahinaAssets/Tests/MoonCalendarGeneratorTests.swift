...
final class MoonCalendarGeneratorTests: XCTestCase {
    ...
    func testMoonPhaseGroupAssignment() {
        /*
         * Verify correct group assignment for each phase range
         */
        let hoonuiPhases = (1...10).map { MoonCalendarGenerator.moonPhase(for: $0) }
        let poepoePhases = (11...16).map { MoonCalendarGenerator.moonPhase(for: $0) }
        let emiPhases = (17...30).map { MoonCalendarGenerator.moonPhase(for: $0) }
        ...
        for phase in poepoePhases {
            XCTAssertEqual(phase.groupName, "Poepoe", "Days 11-16 should be in Poepoe group")
        }

        for phase in emiPhases {
            XCTAssertEqual(phase.groupName, "Emi", "Days 17-30 should be in Emi group")
        }
    }
    ...
    func testPhaseForDateConsistencyAcrossMonth() {
        ...
        /*
         * Over 28 days we expect roughly 27-28 phase changes (since ~29.5 day cycle)
         */
        XCTAssertGreaterThan(phaseChanges, 24, "Should see phase changes through month")
    }
